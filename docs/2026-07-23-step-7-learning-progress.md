...
- 현재 상태: **대화형 학습 진행 중, 1단원 1-1~1-4·2단원 2-1~2-3·3단원 3-1~3-4·4단원 4-1~4-4·5단원 5-1~5-6 사용자 검증 완료, 6단원 시작 대기**
...
4. `[진행 중]` 이 진행표를 기준으로 대화형 학습과 실제 source 확인을 진행한다. 현재 1단원 1-1~1-4, 2단원 2-1~2-3, 3단원 3-1~3-4, 4단원 4-1~4-4와 5단원 5-1~5-6을 완료했고 6단원 시작을 기다린다.
...
아래 목록은 2026-07-24 기준 전체 학습량과 흐름을 파악하기 위한 비확정 예정안이다. 2~5단원은 위 목록으로 확정했으며, 아래 예정안만으로 나머지 단원의 `대기` 상태를 바꾸거나 학습 완료 증거로 사용하지 않는다. 아래 3·4·5단원 행은 최초 예정안을 보존한 이력이고 현재 범위와 상태는 위 확정 목록을 따른다.
...
현재 확정된 학습서·source 수정 대기 항목은 없다. 5단원 시작 전 `useProximity`에서 발견한 pending Promise 뒤 unmount 후보는 5-4에서 실제 비동기 제어와 test 범위를 재검토했고, 늦은 이전 start가 더 새로운 subscription을 제거할 수 있는 소유권 경쟁 조건으로 범위를 구체화해 수정·검증했다. 5-5에서 확인한 direct unmount 뒤 불필요한 state setter 후보는 native resource·표시 UI 영향이 없는 저영향 cleanup 정합성 문제로 확인했으며, 사용자가 학습용 sample 범위에서 현행 source를 유지하기로 결정했다. 4단원 시작 전 발견한 source 수정 후보도 4-3에서 실제 source와 설치 계약을 재검토한 뒤 같은 sample 범위에서 기존 hydration 실패 한계를 인정해 현행 source를 유지하기로 결정했다. 1-4에서 발견한 상태 문구 누락, 2단원 시작 전 전체 source 대조에서 발견한 학습 주석 오류, 3단원 시작 전 상세 화면 header option 누락과 docs·repository 진입 문서 정합성 보완, 5-1 대화형 설명 보강, 5-4 subscription 소유권 수정과 5-5 현행 유지 결정을 아래와 같이 기록한다. 단원 시작 전 보완과 수정 후보 기록은 해당 단원의 학습 시작이나 완료로 계산하지 않는다.
